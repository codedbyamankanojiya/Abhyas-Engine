...
export function Session() {
  ...
  const fetchAIAnalysisFromSectionData = async (questionId: string) => {
    try {
      const response = await fetch(API_ENDPOINTS.sectionData);
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    const fetchQuestions = async () => {
      setLoading(true);
      try {
        const response = await fetch(API_ENDPOINTS.questions(state.roleId, state.level));
        const data = await response.json();
  ...
  const handleSilenceDetected = async () => {
    ...
    if (completeAnswer) {
      try {
        ...
        await fetch(API_ENDPOINTS.questionTranscript, {
          method: 'POST',
          headers: {
    ...
    try {
      ...
      const [socket, audioContext, stream] = await Promise.all([
        ...
        new Promise<WebSocket>((resolve, reject) => {
          const wsUrl = API_ENDPOINTS.sttWs;
      ...
      <AnimatePresence>
        {loading && !hasPlayedFirstQuestion && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">Abhyas Engine is crafting personalized questions for you...</p>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="flex items-center justify-between border-b border-gray-200 bg-white/90 px-4 py-3 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-slate-950/85">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 overflow-hidden rounded-full ring-2 ring-white/70 shadow-md shadow-aiva-purple/20 dark:ring-white/10">
              <img src="/Assets/Laadla.png" alt="Sarthi Assistant" className="h-full w-full object-contain" />
            </div>
            <div>
              <h1 className="text-sm font-medium text-gray-900 dark:text-slate-100">Abhyas Interview Session</h1>
