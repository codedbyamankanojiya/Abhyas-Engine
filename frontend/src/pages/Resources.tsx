...
/* ── Content Parser & Professional Renderer ────────────────── */
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className={`text-xs font-semibold flex items-center gap-1.5 transition-all px-2.5 py-1.5 rounded-xl border ${
        copied
...
function InteractiveQuestion({ qText, options, index }: { qText: string; options: { key: string; text: string; isCorrect: boolean }[]; index: number }) {
  const [selectedKey, setSelectedKey] = useState<string | null>(null);

  return (
    <div className="my-6 p-6 rounded-3xl border border-indigo-500/10 bg-indigo-500/5 backdrop-blur-xl shadow-lg relative overflow-hidden">
      <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none" />
      
      <h4 className="text-base font-extrabold text-gray-900 dark:text-white mb-4 flex items-start gap-3">
        <span className="flex h-7 w-7 items-center justify-center rounded-xl bg-indigo-500 text-xs font-black text-white shrink-0 mt-0.5 shadow-md shadow-indigo-500/20">?</span>
        <span className="leading-relaxed">{qText}</span>
      </h4>

      <div className="grid gap-3 sm:grid-cols-2">
        {options.map((opt) => {
          const isSelected = selectedKey === opt.key;
          const showAnswers = selectedKey !== null;
          
          let cardStyle = "border-gray-200/60 bg-white/40 dark:border-white/5 dark:bg-gray-900/40 text-gray-700 dark:text-gray-300 hover:bg-white/60 dark:hover:bg-gray-900/60 cursor-pointer";
          
          if (showAnswers) {
            if (opt.isCorrect) {
              cardStyle = "border-emerald-500 bg-emerald-500/10 text-emerald-800 dark:text-emerald-300 font-semibold shadow-md shadow-emerald-500/5";
            } else if (isSelected) {
              cardStyle = "border-red-500 bg-red-500/10 text-red-800 dark:text-red-300 font-semibold";
            } else {
              cardStyle = "border-gray-200/30 bg-white/10 dark:border-white/5 dark:bg-gray-900/10 text-gray-400 dark:text-gray-600 opacity-60 cursor-not-allowed";
            }
          }
          ...
          return (
            <button
              ...
              className={`flex items-center justify-between p-4 rounded-2xl border text-left text-sm transition-all duration-200 select-none ${cardStyle}`}
            >
              <span>{opt.key.toUpperCase()}) {opt.text}</span>
              {showAnswers && opt.isCorrect && (
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500 text-[10px] text-white font-bold shadow-md">✓</span>
              )}
              {showAnswers && isSelected && !opt.isCorrect && (
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] text-white font-bold shadow-md">✗</span>
              )}
            </button>
          );
        })}
...
function MaterialContentRenderer({ content }: { content: string }) {
  ...
  let questionOptions: { key: string; text: string; isCorrect: boolean }[] = [];
  let inQuestion = false;

  const renderCodeBlock = (codeLines: string[], lang: string, index: number) => {
    const code = codeLines.join('\n');
    return (
      <div key={`code-${index}`} className="my-6 rounded-2xl border border-white/10 overflow-hidden bg-slate-950/90 shadow-lg">
        <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-slate-900/60">
          <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">{lang || 'Code'}</span>
          <CopyButton text={code} />
        </div>
        <pre className="p-5 overflow-x-auto whitespace-pre font-mono text-[13px] text-emerald-400 dark:text-emerald-300 leading-relaxed">
          <code>{code}</code>
        </pre>
      </div>
    );
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line === '---' || line.match(/^[═─-]{5,}$/)) {
      if (inCodeBlock) {
        elements.push(renderCodeBlock(codeBlockLines, codeLanguage, i));
        codeBlockLines = [];
        inCodeBlock = false;
      }
      if (inQuestion) {
        elements.push(<InteractiveQuestion key={`q-${i}`} qText={currentQuestionText} options={questionOptions} index={i} />);
        questionOptions = [];
        inQuestion = false;
      }
      elements.push(<hr key={`hr-${i}`} className="my-8 border-gray-200/50 dark:border-white/10" />);
      continue;
    }
    ...
    if (inCodeBlock) {
      codeBlockLines.push(lines[i]); // Keep original spacing
      continue;
    }

    // Handle multiple choice questions
    // Format matches: "1. What is..." or "2. Which..."
    ...
    if (inQuestion && optMatch) {
      const isCorrect = optMatch[2].includes('✓');
      const textVal = optMatch[2].replace('✓', '').trim();
      questionOptions.push({
        key: optMatch[1],
        text: textVal,
        isCorrect
      });
      // If we collected 4 options (a,b,c,d), we can output the question
      if (optMatch[1] === 'd') {
        elements.push(<InteractiveQuestion key={`q-${i}`} qText={currentQuestionText} options={questionOptions} index={i} />);
        questionOptions = [];
        inQuestion = false;
    ...
    // Handle sections / numbered headers (e.g. "1: What is a Database...")
    ...
    if (sectionMatch) {
      ...
      elements.push(
        <h3 key={`h3-${i}`} className="text-lg sm:text-xl font-extrabold text-gray-900 dark:text-white mt-8 mb-4 flex items-center gap-2.5 border-b border-gray-100 dark:border-white/5 pb-3">
          <span className="text-indigo-500 font-mono text-base">{sectionMatch[1]}.</span>
    ...
    // Handle big section headers (e.g. "SECTION 1: MULTIPLE CHOICE QUESTIONS" or "🛡️ SECURITY INCIDENT RESPONSE FRAMEWORK")
    if ((line.match(/^[A-Z\d\s:🤖🔒🛡️⏰🥇🥈🥉🧬🧠🎤🎓💻❓!·•-]+$/) && line.length > 5) || line.startsWith('🔒') || line.startsWith('🛡️')) {
      if (inQuestion) {
        elements.push(<InteractiveQuestion key={`q-${i}`} qText={currentQuestionText} options={questionOptions} index={i} />);
        questionOptions = [];
        inQuestion = false;
      ...
      elements.push(
        <div key={`section-h-${i}`} className="my-6 p-4 rounded-2xl bg-gradient-to-r from-aiva-purple/10 to-indigo-500/10 border border-aiva-purple/15 text-center">
          <h4 className="text-sm font-black uppercase tracking-[0.2em] text-aiva-purple dark:text-indigo-400">{line}</h4>
        </div>
      );
      continue;
    }
    ...
    if (line.startsWith('·') || line.startsWith('•') || line.startsWith('-')) {
      ...
      // Split by colon to highlight the title (e.g., "Database: Organized collection...")
      const colonIndex = content.indexOf(':');
      if (colonIndex > 0) {
        const title = content.substring(0, colonIndex).trim();
        const desc = content.substring(colonIndex + 1).trim();
        elements.push(
          <div key={`bullet-${i}`} className="my-3 p-4 rounded-xl border border-gray-200/50 bg-white/40 backdrop-blur-md dark:border-white/5 dark:bg-gray-900/40 hover:border-aiva-purple/30 transition-all shadow-sm">
            <h5 className="text-[14px] font-bold text-indigo-600 dark:text-indigo-400 mb-1">{title}</h5>
            <p className="text-[13px] text-gray-700 dark:text-gray-300 leading-relaxed">{desc}</p>
          </div>
        );
      } else {
        elements.push(
          <div key={`bullet-simple-${i}`} className="my-2.5 flex items-start gap-2.5 pl-2">
            <span className="h-1.5 w-1.5 rounded-full bg-indigo-500 shrink-0 mt-2" />
            <p className="text-[13px] text-gray-700 dark:text-gray-300 leading-relaxed">{content}</p>
          </div>
        );
      }
      continue;
    ...
    // Default paragraph
    elements.push(
      <p key={`p-${i}`} className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed my-3 pl-1">
        {line}
      </p>
    );
  }

  // Push remainder if code block or question is open
  if (inCodeBlock && codeBlockLines.length > 0) {
    elements.push(renderCodeBlock(codeBlockLines, codeLanguage, lines.length));
  }
  if (inQuestion) {
    elements.push(<InteractiveQuestion key={`q-last`} qText={currentQuestionText} options={questionOptions} index={lines.length} />);
  }

  return <div className="space-y-4 max-w-none text-left">{elements}</div>;
}
...
• Capital Account: Capital transfers, investments`
    ...
    return cheatSheets[categoryId] || "📝 Cheat sheet coming soon for this subject! Sarthi is working on it.";
  };
...
• Below 40: Foundation Review Needed`
  ...
  const allMaterials = useMemo(() => [
    ...
    },
    {
      ...
      type: "pdf",
      size: "1.5 MB",
      description: "Curated collection of subjective questions and answer keys.",
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="mt-1 text-lg font-black tracking-tight text-gray-900 dark:text-white">Talk to Sarthi</div>
                          ...
                          <div className="p-8 rounded-[2.5rem] bg-white/70 dark:bg-gray-900/60 backdrop-blur-2xl border border-white/50 dark:border-white/10 shadow-2xl">
                            <div className="flex items-center gap-4 mb-8">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-500 font-medium">Abhyas Curated Resource</p>
                              </div>
                            </div>
                            ...
                            {isContentLoading ? (
                              <div className="py-20 flex flex-col items-center justify-center gap-4">
                                ...
                                />
                                <p className="text-sm font-bold text-gray-400 animate-pulse">Sarthi is preparing your notes...</p>
                              </div>
                            ) : viewingMaterialId === "notes" && resourceContent ? (
                              <MaterialContentRenderer content={resourceContent} />
                            ) : viewingMaterialId === "cheat-sheet" ? (
                              <MaterialContentRenderer content={generateCheatSheet(selected?.id || "")} />
                            ) : viewingMaterialId === "practice-assessment" ? (
                              <MaterialContentRenderer content={generatePracticeAssessment(selected?.id || "")} />
                            ) : (
                              <div className="py-20 text-center">
                                <p className="text-gray-400 font-medium italic">This resource is currently being updated by Sarthi.</p>
                              </div>
                            )}
                          </div>
                        </motion.div>
              ...
              <div className="rounded-[2.5rem] bg-white/40 dark:bg-gray-800/40 backdrop-blur-2xl shadow-xl border border-white/40 dark:border-white/5 p-6 space-y-8">
                ...
                <div className="p-5 rounded-[2rem] bg-aiva-purple/5 border border-aiva-purple/10">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-4 leading-relaxed">
                    Ask Sarthi to create a personalized learning schedule for {selected?.title}.
                  </p>
                  <button 
                    onClick={() => {
